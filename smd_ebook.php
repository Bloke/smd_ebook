...
EOJS;
        ...
        foreach ($files as $file) {
            ...
            if ($ext !== 'opf' || ($ext === 'opf' && $can_opf)) {
                ...
                if ($ext === 'xhtml') {
                    $content_view = '[<a href="#" class="smd_ebook_view">'.gTxt((((strpos($file, '_toc.xhtml') !== false) || (strpos($file, '_end.xhtml') !== false)) ? 'smd_ebook_lbl_view_toc' : 'smd_ebook_lbl_view')).'</a>]';
                } else {
                    if ($ext === 'css') {
                        $has_css = true;
                    }
...
function smd_ebook_create()
{
    ...
    foreach (ps('smd_ebook_articles') as $artid) {
        ...
        if ($row) {
            ...
            if (!isset($firstfile)) {
                $firstfile = $row['url_title'] . '.xhtml';
                $filebase = sanitizeForFile(($outfile === '') ? $row['url_title'] : $outfile);
                $listfile = $filebase . '.smd';
                $notefile = $filebase . '_notes.xhtml';
                $toc_file = $filebase . '_toc.xhtml';
                $ncx_file = $filebase . '.ncx';
                $end_file = $filebase . '_end.xhtml';
                $opf_file = $filebase . '.opf';
                $lmk_file = $filebase . '_landmark.xhtml';
                $cover_file = 'cover.xhtml';
                        ...
                        if ($mime_type) {
                            ...
                            if ($is_epub) {
                                ...
                                if ($ret) {
                                    ...
                                    $reps['{smd_ebook_manifest_cover}'] = '<item id="cover" media-type="application/xhtml+xml" href="cover.xhtml" />'
...
//						$node = mb_convert_encoding(trim($item->nodeValue), 'HTML-ENTITIES', 'utf-8');
            ...
            $article_refs[] = '<item id="'.$ebook_item.'" media-type="application/xhtml+xml" href="'.$row['url_title'].'.xhtml" />';
...
//		$reps['{smd_ebook_spine_ncx}'] = '';
    ...
    if ($is_epub) {
        ...
        $reps['{smd_ebook_md_x}'] = '';
        $reps['{smd_ebook_guide_cover}'] = '<reference type="cover" title="Cover" href="cover.xhtml" />';
...
// ------------------------
// Stage 2 only: Pre-requisites are that the necessary files (toc, .xhtml, ncx + opf)
...
* One .xhtml file for each article you selected.
* Any images you have used in the book. Reveal them by clicking the [Images] link below the chapter in which they are used.
* An HTML Table of Contents with the filename of the first article in the project plus @_toc.xhtml@.
* An author notes page (filename of first article plus @_notes.xhtml@).
